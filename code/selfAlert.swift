...
let ZUBET_MAIN_COLOR = UIColor(red: 52/255.0, green: 197/255.0, blue:
    170/255.0, alpha: 1.0)
let ZUBET_HIGHT_TITLE_COLOR = UIColor.grayColor()
   ...
   func showAlert(title: String, subTitle: String?,buttonTitle: String ,otherButtonTitle:String?,action:((OtherButton: UIButton) -> Void)) {
        ...
        let window: UIWindow = UIApplication.sharedApplication().keyWindow!
            ...
            if buttonTitle.isEmpty == false {
                ...
                button.backgroundColor = ZUBET_MAIN_COLOR
                button.setTitleColor(ZUBET_HIGHT_TITLE_COLOR, forState:.Highlighted)
                button.userInteractionEnabled = true
                button.addTarget(self, action: #selector(ZuberAlert.doCancel(_:)), forControlEvents: UIControlEvents.TouchUpInside)
                button.tag = 0
            ...
            if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
                ...
                button.setTitleColor(ZUBET_HIGHT_TITLE_COLOR, forState:.Highlighted)
                button.backgroundColor = ZUBET_MAIN_COLOR
                button.addTarget(self, action: #selector(ZuberAlert.pressed(_:)), forControlEvents: UIControlEvents.TouchUpInside)
                button.tag = 1
                buttons.append(button)
            }
            resizeAndRelayout()
